<template>
  <el-form ref="formInstace" :model="department" :rules="rules" label-width="150px" class="demo-department" status-icon>
    ...
    <el-form-item>
      <el-button type="primary" @click="handleUpdateEmployee()">Create</el-button>
...
<script lang="ts">
  ...
  export default defineComponent({
    ...
    watch: {
      ...
      idUser: {
        async handler (value: string) {
          if (!value) return;
          const employee = await axios.get(`http://localhost:3000/employee/${value}`);
          if (employee.status === 200)
            this.listCompany = employee.data.companies.this.listDepartment = [employee.data.department];
        },
        immediate: true,
      },
    },
    async created () {
      ...
      // List user-companys
      //   const dataUser_company = await axios.get(`http://localhost:3000/employee/${account.user.id}`);
